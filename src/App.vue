...
import { Archive, Check, ChevronDown, ChevronsUpDown, CircleAlert, CircleCheck, Copy, Database, Download, Edit3, Eye, EyeOff, Folder, History, ImagePlus, Info, LoaderCircle, Plus, RefreshCw, Rocket, Server, Trash2, TriangleAlert, X } from "lucide-vue-next";
...
import { useImageMcp } from "./composables/useImageMcp";
...
const {
  busy: imageMcpBusy, checking: imageMcpChecking, eligible: imageMcpEligible, status: imageMcpStatus,
  blockedByProvider: imageMcpBlockedByProvider,
  label: imageMcpLabel, prompt: imageMcpPrompt, prepare: prepareImageMcp,
  confirm: confirmImageMcp, close: closeImageMcp, refresh: refreshImageMcp,
} = useImageMcp(
  computed(() => workspace.activeProfileId),
  computed(() => workspace.providerBuckets.find((provider) => provider.isCurrent)?.providerId ?? null),
  computed(() => workspace.providerConfigSwitching),
);
...
const initializationState = ref<"checking" | "missing" | "failed" | "ready">("checking");
const initializationError = ref<string | null>(null);
const initializingApp = computed(() => initializationState.value === "checking");
const workspaceBlocked = computed(() => initializationState.value !== "ready" || !workspace.activeProfile);
const gateState = computed(() => initializationState.value === "ready" ? "missing" : initializationState.value);
const clientDownloadUrl = "https://openai.com/zh-Hans-CN/codex/";
...
function handleKeydown(event: KeyboardEvent) {
  ...
  if (imageMcpPrompt.value) {
    closeImageMcp();
  } else if (forcePrompt.value) {
    cancelForcePrompt();
...
function toggleStorageMenu() {
  if (!hasMultipleStorageLocations.value || imageMcpBusy.value) return;
...
async function refreshProviders(showMessage = true, requireSuccess = false) {
  if (providersRefreshing.value) return;
  providersRefreshing.value = true;
  try {
    await workspace.refreshProviders();
    if (requireSuccess && workspace.error) throw new Error(workspace.error);
    ...
    if (profileId) {
      const entries = await Promise.all(workspace.providerBuckets.map(async (provider) => {
        try {
          ...
          return [provider.providerId, { config: { baseUrl: null, apiKeyMasked: null, authStatus: null, configured: false }, apiKey: null, size: 0 }] as const;
        }
      }));
...
async function refreshProvider(provider: ProviderBucket) {
  ...
  try {
    ...
    if (freshBucket?.isCurrent) await refreshImageMcp();
...
async function openConfig(mode: "add" | "edit", provider?: ProviderBucket) {
  if (imageMcpBusy.value) return;
  if (workspaceBlocked.value || !workspace.activeProfile) { notify("未选择配置目录", "warning"); return; }
  if (provider && !workspace.profiles.some((profile) => profile.id === provider.profileId)) { notify("未选择配置目录", "warning"); return; }
...
async function saveConfig() {
  ...
  if (workspaceBlocked.value || !workspace.profiles.some((profile) => profile.id === configDraft.value.profileId)) { notify("未选择配置目录", "warning"); return; }
  if (!configDraft.value.providerId.trim()) { notify("请填写供应商名称", "warning"); return; }
...
async function initializeWorkspace() {
  if (initializationInFlight || disposed) return;
  initializationInFlight = true;
  initializationState.value = "checking";
  initializationError.value = null;
  storageMenuOpen.value = false;
  try {
    await workspace.initialize();
    if (disposed) return;
    if (!workspace.activeProfile) {
      initializationState.value = "missing";
      return;
    }
    // Keep the gate until the discovered directory and all provider details
    // are ready. Ordinary refresh errors must not change this entry state.
    await refreshProviders(false, true);
    if (!disposed) initializationState.value = "ready";
  } catch (error) {
    if (disposed) return;
    initializationError.value = error instanceof Error ? error.message : String(error);
    initializationState.value = "failed";
  } finally {
...
onMounted(() => {
  ...
  if (isTauri()) {
    void listen("app://check-update", () => { void checkUpdate(); }).then((unlisten) => {
      if (disposed) unlisten(); else unlistenTrayUpdate = unlisten;
    }).catch((error) => {
      if (!disposed) notify(`更新通知监听失败：${String(error)}`, "danger");
    });
  }
  void initializeWorkspace();
});
onUnmounted(() => { disposed = true; window.removeEventListener("keydown", handleKeydown); document.removeEventListener("pointerdown", closeStorageMenuFromOutside); unlistenTrayUpdate?.(); messages.dispose(); });
</script>
...
<template>
  <div class="app-frame" @contextmenu.prevent>
    ...
    <main class="app-content" :aria-busy="initializingApp ? 'true' : 'false'">
      <template v-if="!workspaceBlocked">
        <section ref="storageRoot" class="storage-bar">
          <button type="button" class="storage-select" data-testid="storage-location-switch" :disabled="!hasMultipleStorageLocations || imageMcpBusy" :aria-expanded="storageMenuOpen && hasMultipleStorageLocations" aria-haspopup="menu" @click="toggleStorageMenu">
    ...
    <ModalDialog v-if="imageMcpPrompt && !workspaceBlocked" :title="imageMcpPrompt.mode === 'repair' ? '修复生图' : '生图配置完成'" :close-disabled="imageMcpBusy" @close="closeImageMcp"><p>{{ imageMcpPrompt.mode === 'repair' ? '修复生图会关闭 ChatGPT 客户端。请先保存未完成的工作，修复完成后将重新启动。' : '重启 ChatGPT 后生图功能才能生效。' }}</p><template #actions><div class="modal-actions"><button class="secondary-button" :disabled="imageMcpBusy" @click="closeImageMcp">取消</button><span class="spacer" /><button class="primary-button" :disabled="imageMcpBusy" @click="confirmImageMcp"><LoaderCircle v-if="imageMcpBusy" :size="15" class="spinning" /><span>{{ imageMcpBusy ? '处理中…' : imageMcpPrompt.mode === 'repair' ? '重启并修复' : '立即重启' }}</span></button></div></template></ModalDialog>
